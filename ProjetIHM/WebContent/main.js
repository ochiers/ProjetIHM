...
function create() {
	...
	textTimeRemaing = game.add.text(32, 32, "Time Remaining", { font: "24px Arial", fill: "green"});
	textRatioScore = game.add.text(32, 64, "Ratio", { font: "24px Arial", fill: "green"});

    //balloon = game.add.sprite(game.world.centerX, game.world.centerY, 'balloon');
    ...
    /*for (var x = 0; x < 75; x++)
    {
    	var b = game.add.sprite(Math.floor((Math.random() * (maxWidth-100)) + 1), 50 + Math.floor((Math.random() * 4000) + 600), 'balloon1');
...
function endGame(){
	...
	for(var i = 0 ;i <ballons.length; i++){
		if(ballons[i].body != null)
			game.add.tween(ballons[i]).to( { alpha: 0 }, 1000, Phaser.Easing.Linear.None, true);
	}
	game.time.events.add(Phaser.Timer.SECOND * 3, function(){for(var i = 0 ;i <ballons.length; i++)ballons[i].destroy()}, this);
}
...
function shoot(mouseEvent){
	...
	var point = {x: mouseEvent.clientX, y: mouseEvent.clientY};
	var mis = 0;
	for(var i=0; i<ballons.length;i++){
		var b = ballons[i];
		if(b.body!=null){
			var rectangle = {x1: b.body.position.x, x2: b.body.position.x + b.body.width, y1: b.body.position.y, y2: b.body.position.y + b.body.height};
			if(pointRectangleIntersection(point, rectangle)){
				var p = game.add.sprite(b.body.position.x,b.body.position.y, 'puzzle'+Math.floor(Math.random()*2 + 1));
...
function createBalloons () {
    ...
    for (var x = 0; x < 10; x++)
    {
        var alien = aliens.create(100, 50 + Math.floor((Math.random() * 100) + 1) , 'balloon');
...
function createBallonsPeriodicaly(){
	...
	if(timeRemaining > freq){
		...
		game.time.events.add(Phaser.Timer.SECOND * freq, createBallonsPeriodicaly, this);
	}
